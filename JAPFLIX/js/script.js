...
document.addEventListener('click', function(evento) {
    ...
    if (peliculaElement) {
        ...
        detalles.style.display = 'block'; // Muestra el accordion.

        // Obtener la película.

        const pelicula = peliculas.find(p => p.title === peliculaElement.querySelector('h2').innerText);

        // Cargar información de la película en el accordion.
        ...
        segundoBloque.innerHTML = `
            ...
            <p><strong>Costo:</strong>$${pelicula.budget}</p>
...
// Cerrar el accordion.
    ...
    if (evento.target.classList.contains('close-button')) {
        const detalles = document.getElementById('detallesPelicula');
        detalles.style.display = 'none'; // Oculta el accordion.
    }
});